                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="ml-3 flex h-7 items-center">
                          <button
                            type="button"
                            onClick={onClose}
                            className="relative rounded-lg bg-gray-50 p-1.5 text-gray-400 ring-1 ring-gray-200 transition-all hover:bg-gray-100 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-950"
                          >
                    ...
                    <div className="relative mt-8 flex-1 px-4 sm:px-6">
                      <nav className="space-y-1">
                        {links.map((link, index) => (
                          link.is_divider ? (
                            <hr key={`divider-${index}`} className="my-4 border-gray-200" />
                          ) : (
                            <Link
                              key={link.href}
                              href={link.href}
                              onClick={onClose}
                              className="block rounded-xl px-4 py-3 text-base font-medium text-gray-700 transition-all hover:bg-gray-50 hover:text-gray-950 hover:shadow-sm"
                            >
                              {link.label}
                            </Link>
                          )
